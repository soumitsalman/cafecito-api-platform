...
- Flattened intelligence fields, with a small stable resource core.
...
Industry APIs use different meanings for count-like fields. Espresso uses
`pagination.num_results` for the number of records in the current page and does
not calculate a separate total-match count. Clients can also obtain the page
length from `data.length`.
...
Every Event or Signal item has a stable core plus actual flattened intelligence
...
| summary | Normalized narrative projection | Optional normalized projection. |
| data_quality | Derived | Present only for a known quality condition. |

The API also returns every non-empty optional intelligence member at the item root. Canonical core fields win on
a name conflict. The API exposes kind as a flattened core field; it never exposes embedding, relation direction, or a nested internal object.
...
### 3.4 Intelligence field model

The response item is the union of flattened core fields (id, kind, created_at,
url, base_url, source_id, source, and tags) and the optional intelligence
fields available for that Event or Signal. The complete field set varies by
record; clients must not require every optional field to be present.

The following intelligence fields are common fields, not a closed schema:
...
The stored tags column is canonical. If another optional intelligence field also contains tags, the response emits
one reconciled top-level tags value rather than two fields.
...
| R12 | GET /entities | Discovers company and people names available for Event filtering. | Industry pattern adapted from GDELT Entities and PredictHQ entity filtering. | Current-data + Query-change |
| R13 | GET /regions | Discovers region names available for Event filtering. | Industry pattern adapted from GDELT geography discovery and PredictHQ Places. | Current-data + Query-change |
...
| event_types | Exact Event type names in normalized snake_case, for example `policy_change,market_entry`. |
| categories | Exact category names in normalized snake_case, for example `regulation,technology`; this is a separate field from `event_types`. |
| impact_levels | Exact impact-level names, for example `high,medium`. |
| companies, people, products, regions | Exact names normalized to snake_case, for example `microsoft,nvidia`, `sam_altman,elon_musk`, `windows,geforce`, or `north_america,europe`. |
...
| impact_levels | Exact Signal impact-level names, for example `high,medium`. |
| impacted_domains | Exact impacted-domain names normalized to snake_case, for example `public_health,climate`. |
| source_ids | Direct Signal Source UUIDs. |
| tags | Fuzzy text matching against persisted Signal tag labels. |
| q | Semantic query. |
...
| event_types | Exact Event type names normalized to snake_case. |
| categories | Exact category names normalized to snake_case; a separate field from `event_types`. |
| impact_levels | Exact impact-level names. |
| companies, people, products, regions | Exact Event names normalized to snake_case. |
| entities | Exact Event company or people names normalized to snake_case. |
| source_ids | Direct Event Source UUIDs. |
| tags | Fuzzy text matching against persisted Event tag labels. |
| q | Semantic query. |
...
These routes expose public filter names in normalized snake_case where applicable.
They do not provide canonical entity profiles, aliases, countries, coordinates,
or a geography hierarchy. Tag discovery returns labels used by fuzzy matching.
...
| Payload | kind is exposed; canonical source/URL/summary mapping is incomplete | Hide kind; emit stable core plus flattened intelligence fields. |
