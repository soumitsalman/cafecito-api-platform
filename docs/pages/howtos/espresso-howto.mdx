...
[Espresso](https://espresso.cafecito.tech) is Cafecito's curated business intelligence product suite. The API exposes the structured data behind Espresso as read-only **sips** — event digests, derived signals, tags, and relationships ready for dashboards, monitoring workflows, and AI agents.
...
- **Relationship traversal** - Follow `SAME_AS` and `DERIVED_FROM` links via `GET /events/{id}/evidence` and `GET /signals/{id}/events`
...
| `acc` | number (0–1) | Minimum similarity for `q` — `0.0` = broad, `1.0` = strict (default `0.5`) |
...
```text
reported:2026-06-07
related:public_policy|market_trends|regulation
...
future_outlook:Short-term regulatory scrutiny will intensify...
```
...
Use `response_type=text` on list endpoints for token-efficient agent consumption. See the [MCP Integration guide](/howtos/mcp-howto) for client setup patterns.
...
events?.forEach((e) => console.log(e.id, e.reported, e.site_name, e.briefing));
```
...
for event in resp.json() or []:
    print(event.get("id"), event.get("reported"), event.get("site_name"), event.get("briefing"))
```
