---
title: Troubleshooting Cafecito APIs
description: Diagnose authentication, validation, pagination, rate-limit, empty-result, and service failures for Beans and Espresso.
---
...
| Cursor no longer accepted | Token expired or was altered | Start the collection again with the same filters | After a fresh first page | Both products omit `pagination.cursor`; continue with `next_cursor` |
| Unexpected format | Unsupported `response_type` | Use JSON | No | Espresso `response_type=json` |
