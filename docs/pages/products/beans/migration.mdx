---
title: Migrate to the Beans Publisher Content API
description: Map TheNewsAPI, World News API, GNews, finlight, NewsAPI.ai, and NewsData.io workflows to Beans Articles, Sources, Stories, and feeds.
---

# Migrate to Beans

Beans is a read-only publisher-content API for news, blogs, earnings and financial reports, litigation, official statements, research, technical documents, and coverage context. It is designed for workflows that need publisher text and metadata. It is not a business-intelligence API; use [Espresso](/products/espresso) for Events, Signals, evidence, and synthesized conclusions.

These provider names are comparison patterns, not a claim of identical fields or content rights.
...
Behavioral differences: Beans uses cursor pagination, UUID identifiers, strict query validation (unknown parameters return HTTP 400), and response-only `post`. Full-content is requested, not guaranteed.
...
`post` may appear on Article responses. `content_type=post` returns HTTP 400. Omit `content_type` to include all stored types. Provider labels such as `pr` are not aliases.
...
- Error responses use `{ "error": { "code", "message" } }` with HTTP 400, 401, 404, 429, or 500 as applicable.
