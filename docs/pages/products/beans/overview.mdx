---
title: Overview
description: Search publisher Articles, Sources, Stories, and external observations with the Beans REST API and MCP server.
---
...
Beans is a read-only publisher-content API for news applications, research workflows, and AI agents. It returns publisher Articles, Source metadata, chronological and attention-ranked feeds, related publisher reading, external Article mentions, and stable Story groups.

- **Articles** are publisher items: news, blogs, reports, podcasts etc.
- **Sources** are publisher records available on an Article or through the Source routes.
- **Stories** group related Articles under a stable UUID and expose the member-Article route.
- **Mentions** are external platform or forum observations for a selected Article.
- **Discovery routes** return normalized values that can be used as Article and Story filters.

Use Espresso when the question asks for a structured Event, Signal, impact, forecast, or evidence relationship. Beans reports publisher material and its coverage context; it does not turn an Article or attention metric into an Espresso conclusion.
...
- **Content on demand**: keep collections compact and request `full_content=true` only for a selected Article or a narrowed Article collection.
...
| User needs | Start here | Follow only when needed | Provider parallel |
| --- | --- | --- | --- |
...
All public Beans routes use the `/beans` prefix and the same Bearer API key used by other Cafecito products.
...
The following requests make the same Article search. The query asks for publisher material rather than an Espresso Event or Signal. `score_threshold` is supplied with `q` so the request also works on the feed routes that require the threshold when a query is present.
...
Expect HTTP `400` for invalid input, `401` for a missing or invalid key, `404` for a selected record that does not exist, `429` when request limits are reached, and `500` when the service is unavailable. A successful response does not include a duplicate status field.
...
| `content` | optional string | Included only when `full_content=true` is requested and content is available. |
...
| --- | --- | --- |
| `GET /beans/health` | Health object | None |
...
| `GET /beans/sources` | Source collection envelope | `q`, `domains`, `limit`, `cursor` |
| `GET /beans/sources/{id}` | Source detail envelope | `id` path parameter |
| `GET /beans/mcp` | MCP tool transport | No REST filter parameters; connect with the same API key. |
...
| Relevance threshold | `score_threshold` is in the `0.0` through `1.0` range. Use it only with `q`. Article feed routes require a non-zero `score_threshold` when `q` is present; Story discovery rejects a threshold without `q`. |
| Content projection | `full_content=true` requests available Article content without changing which records match. |
...
| `content_type` | One stored Article type. Omit to include every stored type. | Article search, latest, trending, similar, Stories, Story Articles. |
...
`content_type` accepts every stored Beans type. Omit it when a collection should include all stored types:
...
Do not translate provider labels such as `pr` into Beans aliases. Use one of the published values exactly.
