...
| `content` | optional string | Included only when `full_content=true` is requested and Beans has body content. This is not a universal full-text guarantee or a grant of republication rights. Handle a missing value, preserve `url` for attribution, and follow the [Third-Party Content and Attribution Policy](/company/content-rights-policy). |
