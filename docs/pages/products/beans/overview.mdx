---
description: Search public information across news, blogs, earnings, litigation, official statements, research, and technical documents with the Beans API and MCP server.
---
...
Cafecito collects data from nearly 15,000 different public-information and publicly available sources/outlets daily. Coverage varies and does not imply source endorsement, complete coverage, or unrestricted content rights.
...
| `content` | optional string | Included only when `full_content=true` is requested and Beans has body content. This is not a universal full-text guarantee or a grant of republication rights. Handle a missing value, preserve `url` for attribution, and follow the [Third-Party Content Policy](https://cafecito.tech/docs/third-party-content-policy/). |
...
| --- | --- | --- |
| Source | `id`, `url`, `domain`, `name`, optional `description`, `favicon_url`, `rss_feed_url` | Source metadata. Preserve `id` and use it with the `sources` Article or Story filter. |
...
`GET /beans/sources/{id}` returns one Source detail envelope. Optional source-description, favicon, and source-reference fields can be absent; an unavailable field is not an error.
