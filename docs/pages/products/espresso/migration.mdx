---
title: Migration
description: Migrate from article, event, story, or intelligence feeds to Espresso Events, Signals, evidence, and agent-ready formats.
---

# Migrate to Espresso

Espresso is a business-intelligence API for answering two different questions:

- **What happened?** Search **Events** for concrete developments.
- **What does it mean?** Search **Signals** for synthesized conclusions, then retrieve the supporting Events when the answer needs substantiation.

It is not a drop-in replacement for an article archive or a generic entity database. Plan the migration around the question your application or agent must answer, then carry returned IDs into the next call.
...
## Adapt common provider patterns

These mappings describe the typical adaptation when moving from similar intelligence services. Validate field availability against the [Espresso API reference](/api/espresso) before changing production parsers.

| Existing service pattern | Espresso adaptation | Important difference |
| --- | --- | --- |
| **GDELT-style event or theme search** | Search Events with `q`, fuzzy `tags`, and exact Event filters. | `tags` are fuzzy. Structured values such as `companies`, `regions`, and `event_types` use exact normalized values. |
| **PredictHQ-style structured event monitoring** | Search Events by company, people, product, region, category, type, or impact level. | `from` and `to` filter Espresso record `created_at`; they are not occurrence, lifecycle, or forecast dates. |
| **Perigon-style article or story retrieval** | Use Events for concrete developments and Signals for synthesized conclusions. | Espresso does not expose article bodies or a story-cluster substitute through these routes. |
| **Publisher or domain filtering** | Resolve a Source, then pass its UUID as `source_ids` to Event routes. | `GET /espresso/sources?q=...` is case-insensitive source-metadata matching, not semantic retrieval. |
...
- Send a non-null `pagination.next_page` unchanged as the next request `page`. Never construct, decode, or sort the token.
...
4. Replace offset or cursor construction with pass-through use of `pagination.next_page`.
