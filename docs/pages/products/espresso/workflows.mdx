---
title: Workflows
description: Compose Espresso Events, Signals, evidence, Sources, discovery, and pagination into multi-call agent workflows.
---
...
| Treat `pagination.next_page` as opaque. | Send it back unchanged as `page`; `num_results` is only the count for the current page. |
...
2. Reissue the same search with `page=pagination.next_page`.
3. Continue only while `next_page` is non-null.
...
print({"enriched_events": enriched_events, "next_page": next_page})
```
...
`pagination.num_results` is not a total-match count. Do not infer a global total from it or attempt to manufacture the next token.

## Scenario 6: Use MCP with token-optimized results
