---
title: Espresso Events and Signals API
description: Market, business, event, and news intelligence API for searching Events, interpreting Signals, and tracing evidence and Sources.
---
...
**In one line:** Espresso is a market and business intelligence API for searching concrete Events, interpreting Signals, and tracing evidence and Sources for monitoring, research, risk analysis, and AI-agent workflows.

Use it as event intelligence and news intelligence: what happened, what it may mean for a company or market, and what evidence supports that view. Cafecito resource names remain Events, Signals, Evidence, and Sources. Cortado and Latte are future products and are not live APIs. There is no official SDK; integrate over REST or MCP.
...
Use JSON when a client needs strict JSON parsing. Use YAML when readable indentation is more useful. Use TOON when an agent needs the same fields in the most compact representation. Collection pagination is `limit`, `num_results`, and `next_cursor` in every format. Send `pagination.next_cursor` unchanged as the next request `cursor`.
