...
Use JSON when a client needs strict JSON parsing. Use YAML when readable indentation is more useful. Use TOON when an agent needs the same fields in the most compact representation. `pagination.next_cursor` remains opaque in every format: send it unchanged as the next request `cursor`.
